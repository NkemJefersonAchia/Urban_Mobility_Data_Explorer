...
CORS(
    ...
    resources={r"/api/*": {"origins": ["http://127.0.0.1:5500", "http://127.0.0.1:5501", "http://localhost:5500", "http://127.0.0.1:5000"]}}
)
...
def get_trip_details(trip_id):
    ...
    try:
        trip = db_handler.get_trip_by_id(trip_id)
        if trip:
            return jsonify(trip)
...
@login_required
def get_top_routes():
    """Get most popular routes with improved error handling"""
    try:
        ...
        return jsonify(routes if routes else [])
    except Exception as e:
        print(f"Error in top-routes API: {e}")
        return jsonify({'error': 'Failed to fetch top routes'}), 500
