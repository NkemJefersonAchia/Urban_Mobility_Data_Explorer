...
class DatabaseHandler:
    ...
    def __init__(self):
        ...
        print(f"DEBUG: Looking for database at: {self.db_path}")
    ...
    def get_trip_by_id(self, trip_id):
        """Fetch a single trip by its row ID"""
        query = "SELECT rowid as id, * FROM trips WHERE rowid = ?"
        result = self.execute_query(query, (trip_id,))
    ...
    def get_trips(self, limit=100, offset=0, borough=None, min_fare=None, max_fare=None,
        ...
        if hour is not None and hour != '':
            # FIX: Use strftime instead of missing pickup_hour column
            query += " AND strftime('%H', tpep_pickup_datetime) = ?"
    ...
    def get_hourly_patterns(self):
        """Get trip patterns by hour of day (Fixed SQL)"""
        query = """
        SELECT 
    ...
    def get_top_routes(self, limit=10):
        """Get most popular routes (Fixed Aliases)"""
        query = """
        SELECT 
    ...
    def get_speed_analysis(self):
        """Get speed analysis by hour (Fixed missing column)"""
        query = """
        SELECT 
